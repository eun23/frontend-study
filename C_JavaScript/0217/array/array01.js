...
//? 배열의 이름은 여러 개의 데이터를 담는 특성을 살려 복수 형태로 작성을 권장
...
let sancks = ['칸쵸', '초코송이', '포테토칩', '초코송이'];
// : 중복된 요소값 저장 가능
// : 중복된 요소값 중 첫 번째 데이터의 인덱스만 반환
let Index = sancks.indexOf('초코송이');
let notFound = sancks.indexOf('오예스');
console.log(Index); // 1
...
// : 요소가 없으면 -1을 반환
let lastIndexOfSnack = sancks.lastIndexOf('초코송이');
console.log(lastIndexOfSnack); // 3

//? includes()
// : 배열에 특정 요소가 존재하는지 확인
// : 그 결과를 불리언 값으로 반환
//  (요소의 인덱스 x, 존재 유무만 확인)
let hasPotato = sancks.includes('포테토칩');
...
fruits.splice(1, 0, 'Mango', 'peach'); // [ 'Strawberry', 'Mango', 'peach' ]
console.log(fruits);
