...
// : 회원을 검색(findMember)
// : 회원을 제거(removeMember)
// : 활성화된 회원만 필터링(filterActiverMember)
...
// 2. 회원 나이 평균 계산하기
// : Membership 클래스에 geAverageAge 메서드를 추가
...
class Member {
  ...
  constructor(
    public name: string, 
    public age: number, 
    public active: boolean = true // 클래스의 속성에 기본값 지정, 기본 매개변수
    ) {}
}
...
class VIPMember extends Member {
  constructor(name: string, age: number, public rewardPoints: number, active: boolean = true) {
    ...
    // : 부모 클래스의 기본 속성을 초기화하는 동시에 rewardPoints라는 새로운 속성을 추가
...
class Membership<T extends Member> {
  ...
  // active 속성이 false인 회원의 값이 !부정 연산을 통해 true로 변경되어 반환
  return this.members.filter(member => !member.active);
  }
...
// 회원관리 로직이 하나 생성(일반 회원 & VIP 멤버 회원 관리)
...
console.log(membership.findMember('이도경')); // Member { name: '이도경', age: 30, active: false }
console.log(membership.findMember('한동희')); // undefined
